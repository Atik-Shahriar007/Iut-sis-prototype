...
const availableCourses: Course[] = [
  { code: "CSE 4201", name: "Data Mining", faculty: "CSE", credit: 3, section: "A", instructor: "Dr. Samiul Karim", seats: 14 },
  { code: "CSE 4203", name: "Human Computer Interaction", faculty: "CSE", credit: 3, section: "B", instructor: "Dr. Nusrat Jahan", seats: 7 },
  { code: "CSE 4205", name: "Cloud Computing", faculty: "CSE", credit: 3, section: "A", instructor: "Dr. Fahim Rahman", seats: 22 },
  { code: "HUM 4207", name: "Technology & Society", faculty: "HUM", credit: 2, section: "C", instructor: "Md. Mahmud Hasan", seats: 31 },
];

const currentCourses = [
  { code: "CSE 4101", name: "Computer Networks", type: "Core", day: "Sunday", time: "09:00 — 10:30", room: "Academic Block 3 · 402", tone: "blue" },
  { code: "CSE 4103", name: "Software Engineering", type: "Core", day: "Monday", time: "11:00 — 12:30", room: "Academic Block 2 · 201", tone: "mint" },
  { code: "CSE 4105", name: "Artificial Intelligence", type: "Core", day: "Tuesday", time: "09:00 — 10:30", room: "Academic Block 3 · 305", tone: "gold" },
  { code: "HUM 4107", name: "Professional Ethics", type: "Elective", day: "Wednesday", time: "02:00 — 03:30", room: "Academic Block 1 · 107", tone: "rose" },
];

function IUTMark({ dark = false }: { dark?: boolean }) {
  return (
    <div className={`brand-lockup ${dark ? "brand-lockup-dark" : ""}`}>
      <div className="brand-mark">IUT</div>
      <div>
      ...
      <section className="login-panel">
        <div className="login-form-wrap">
          ...
          <form onSubmit={(event) => { event.preventDefault(); onLogin(); }}>
            ...
            <div className="input-wrap"><UserRound size={17} /><input id="student-id" defaultValue="2021-CSE-042" placeholder="e.g. 2021-CSE-042" /></div>
...
function Dashboard({ setView }: { setView: (view: View) => void }) {
  return <>
    <PageHeader eyebrow="SUNDAY · 08 MARCH 2026" title="Good morning, Farzana" description="Here’s your academic snapshot for the Spring 2026 semester." action={<button className="outline-button" onClick={() => setView("Profile")}><UserRound size={16} /> View profile</button>} />
    <div className="stat-grid"><StatCard icon={GraduationCap} label="Current CGPA" value="3.74" detail="+0.12 from last semester" tone="blue" progress={78} /><StatCard icon={BookOpen} label="Registered credits" value="15 / 18" detail="3 credits remaining" tone="gold" progress={83} /><StatCard icon={CalendarDays} label="Classes this week" value="12" detail="4 sessions today" tone="mint" /><StatCard icon={Activity} label="Average attendance" value="91%" detail="Above your 80% target" tone="rose" progress={91} /></div>
    <div className="dashboard-grid"><section className="surface-card next-class-card"><div className="card-heading"><div><span className="eyebrow">UP NEXT · IN 42 MINUTES</span><h2>Computer Networks</h2><p>CSE 4101 · Section A</p></div><span className="live-dot"><i /> Starting soon</span></div><div className="next-class-meta"><div><Clock3 size={17} /><span>09:00 — 10:30</span></div><div><CalendarDays size={17} /><span>Sunday, 08 March</span></div><div><BookOpen size={17} /><span>Academic Block 3 · Room 402</span></div></div><div className="next-class-footer"><div className="instructor"><div className="avatar avatar-tiny">SK</div><span><small>Instructor</small><strong>Dr. Samiul Karim</strong></span></div><button className="soft-button" onClick={() => setView("Schedule")}>See full schedule <ArrowRight size={15} /></button></div></section><section className="surface-card announcement-card"><div className="card-heading"><div><span className="eyebrow">FROM CAMPUS</span><h2>Important updates</h2></div><button className="icon-button"><MoreHorizontal size={18} /></button></div><div className="announcement-item"><span className="announcement-mark gold"><Bell size={15} /></span><div><strong>Course registration closes 12 March</strong><p>Review your course selection before the deadline.</p><small>2 hours ago</small></div></div><div className="announcement-item"><span className="announcement-mark blue"><ClipboardList size={15} /></span><div><strong>Midterm schedule is now available</strong><p>Check your updated calendar for exam dates.</p><small>Yesterday</small></div></div><button className="link-button" onClick={() => setView("Courses")}>View all announcements <ArrowRight size={15} /></button></section></div>
    <div className="bottom-grid"><section className="surface-card progress-card"><div className="card-heading"><div><span className="eyebrow">ACADEMIC JOURNEY</span><h2>Degree progress</h2></div><button className="text-button" onClick={() => setView("Results")}>View record</button></div><div className="degree-progress"><div className="progress-ring"><div><strong>68%</strong><span>complete</span></div></div><div className="progress-legend"><div><span className="legend-dot blue-dot" /><span>Completed credits</span><strong>102</strong></div><div><span className="legend-dot pale-dot" /><span>Remaining credits</span><strong>48</strong></div><div><span className="legend-dot gold-dot" /><span>Expected graduation</span><strong>2026</strong></div></div></div></section><section className="surface-card deadlines-card"><div className="card-heading"><div><span className="eyebrow">YOUR WEEK</span><h2>Upcoming deadlines</h2></div><button className="icon-button"><MoreHorizontal size={18} /></button></div><div className="deadline"><div className="date-block"><strong>10</strong><span>MAR</span></div><div><strong>Software Engineering quiz</strong><p>Tuesday · 11:00 AM</p></div><span className="deadline-tag">In 2 days</span></div><div className="deadline"><div className="date-block"><strong>12</strong><span>MAR</span></div><div><strong>Course registration closes</strong><p>Thursday · 11:59 PM</p></div><span className="deadline-tag urgent">Important</span></div></section></div>
  </>;
}
...
function Courses({ registered, setRegistered, notify }: { registered: string[]; setRegistered: (courses: string[]) => void; notify: (message: string) => void }) {
  ...
  return <><PageHeader eyebrow="ACADEMIC PLANNING" title="Course registration" description="Build your Spring 2026 schedule with confidence." action={<button className="primary-button" onClick={() => notify("Registration changes saved successfully.")}><Check size={16} /> Save changes</button>} /><div className="registration-banner"><div className="banner-icon"><ClipboardList size={20} /></div><div><strong>Registration is open</strong><p>Choose up to 18 credit hours. The add/drop window closes on 12 March 2026.</p></div><div className="banner-progress"><span>{registered.length + 3} / 6 courses</span><div><i style={{ width: `${((registered.length + 3) / 6) * 100}%` }} /></div></div></div><div className="course-toolbar"><div className="search-field"><Search size={17} /><input placeholder="Search courses, code, or instructor" /></div><button className="filter-button">All departments <ChevronDown size={15} /></button><span className="result-count">{availableCourses.length} available courses</span></div><div className="course-layout"><section className="surface-card course-table-card"><div className="table-heading"><div><span className="eyebrow">AVAILABLE THIS SEMESTER</span><h2>Course catalogue</h2></div><button className="icon-button"><MoreHorizontal size={18} /></button></div><div className="course-table"><div className="course-row course-row-head"><span>COURSE</span><span>CREDIT</span><span>SECTION</span><span>INSTRUCTOR</span><span /></div>{availableCourses.map((course) => { const isRegistered = registered.includes(course.code); return <div className="course-row" key={course.code}><div className="course-name"><span className="course-code">{course.code}</span><strong>{course.name}</strong><small>{course.faculty} department</small></div><span>{course.credit} credits</span><span><b className="section-pill">{course.section}</b></span><span className="instructor-cell">{course.instructor}</span><button className={isRegistered ? "registered-button" : "small-outline-button"} onClick={() => toggle(course)}>{isRegistered ? <><Check size={14} /> Added</> : "Register"}</button></div> })}</div></section><aside className="surface-card selection-card"><div className="table-heading"><div><span className="eyebrow">YOUR SELECTION</span><h2>Registration summary</h2></div></div><div className="credit-total"><span>Current load</span><strong>{registered.length + 9}<small> / 18 credits</small></strong><div className="credit-progress"><i style={{ width: `${((registered.length + 9) / 18) * 100}%` }} /></div><small>{18 - registered.length - 9} credits remaining</small></div><div className="selected-list">{[...currentCourses.map((course) => course.code), ...registered].map((code) => { const course = availableCourses.find((item) => item.code === code) || currentCourses.find((item) => item.code === code); return <div className="selected-course" key={code}><span><strong>{code}</strong><small>{course?.name}</small></span><b>{"credit" in (course || {}) ? `${(course as Course).credit} cr` : "3 cr"}</b></div>; })}</div><button className="soft-button full-width" onClick={() => notify("Your registration summary is ready to review.")}>Review registration <ArrowRight size={15} /></button></aside></div></>;
}

function Schedule() {
  const days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday"];
  const blocks = ["09:00", "10:30", "11:00", "12:30", "02:00", "03:30"];
  return <><PageHeader eyebrow="SPRING 2026 · WEEK 10" title="Class schedule" description="Your week at a glance, with room and instructor details." action={<button className="outline-button"><CalendarDays size={16} /> Export calendar</button>} /><div className="schedule-tabs"><button className="active">Week view</button><button>Day view</button><span>08 — 12 March 2026 <ChevronDown size={15} /></span></div><section className="surface-card timetable-card"><div className="timetable"><div className="time-column"><span /><span>{blocks[0]}</span><span>{blocks[1]}</span><span>{blocks[2]}</span><span>{blocks[3]}</span><span>{blocks[4]}</span><span>{blocks[5]}</span></div>{days.map((day) => <div className="day-column" key={day}><div className="day-head"><strong>{day.slice(0, 3).toUpperCase()}</strong><span>{day === "Sunday" ? "08" : day === "Monday" ? "09" : day === "Tuesday" ? "10" : day === "Wednesday" ? "11" : "12"}</span></div><div className="day-grid"><div className={`calendar-class class-blue ${day === "Sunday" ? "placed" : "empty"}`}><strong>Computer Networks</strong><span>09:00 — 10:30</span><small>AB3 · 402</small></div><div className={`calendar-class class-mint ${day === "Monday" ? "placed" : "empty"}`}><strong>Software Engineering</strong><span>11:00 — 12:30</span><small>AB2 · 201</small></div><div className={`calendar-class class-gold ${day === "Tuesday" ? "placed" : "empty"}`}><strong>Artificial Intelligence</strong><span>09:00 — 10:30</span><small>AB3 · 305</small></div><div className={`calendar-class class-rose ${day === "Wednesday" ? "placed" : "empty"}`}><strong>Professional Ethics</strong><span>02:00 — 03:30</span><small>AB1 · 107</small></div></div></div>)}</div></section><div className="schedule-note"><Bell size={17} /><span><strong>Next class:</strong> Computer Networks begins at 09:00 in Academic Block 3, Room 402.</span></div></>;
}

function Results() {
  return <><PageHeader eyebrow="ACADEMIC RECORD" title="Results & progress" description="A clear view of your semester performance and degree journey." action={<button className="outline-button"><ClipboardList size={16} /> Download transcript</button>} /><div className="results-summary"><div className="result-hero"><span className="eyebrow eyebrow-light">CURRENT CGPA</span><strong>3.74</strong><span>Excellent standing · Top 12% of your batch</span><div className="result-rule"><i /></div></div><div className="result-mini"><span className="stat-icon gold"><GraduationCap size={18} /></span><span><small>Completed credits</small><strong>102</strong></span></div><div className="result-mini"><span className="stat-icon mint"><BookOpen size={18} /></span><span><small>Semesters completed</small><strong>06</strong></span></div><div className="result-mini"><span className="stat-icon rose"><Activity size={18} /></span><span><small>Academic status</small><strong>Good standing</strong></span></div></div><section className="surface-card results-table-card"><div className="table-heading"><div><span className="eyebrow">SEMESTER RESULTS</span><h2>Spring 2026</h2></div><button className="filter-button">Select semester <ChevronDown size={15} /></button></div><div className="results-table"><div className="result-row result-row-head"><span>COURSE</span><span>CREDITS</span><span>GRADE</span><span>GRADE POINT</span><span>STATUS</span></div>{[{ code: "CSE 4101", name: "Computer Networks", credit: "3.0", grade: "A-", point: "3.67" }, { code: "CSE 4103", name: "Software Engineering", credit: "3.0", grade: "A", point: "4.00" }, { code: "CSE 4105", name: "Artificial Intelligence", credit: "3.0", grade: "B+", point: "3.33" }, { code: "HUM 4107", name: "Professional Ethics", credit: "2.0", grade: "A", point: "4.00" }].map((row) => <div className="result-row" key={row.code}><span className="course-name"><strong>{row.code}</strong><small>{row.name}</small></span><span>{row.credit}</span><span className="grade-badge">{row.grade}</span><span>{row.point}</span><span className="status-pass"><Check size={14} /> Passed</span></div>)}</div><div className="semester-footer"><span>Semester GPA <strong>3.76</strong></span><span>Total credits <strong>11.0</strong></span><span>Last updated 02 March 2026</span></div></section></>;
}

function Attendance() {
  const courses = [{ code: "CSE 4101", name: "Computer Networks", percent: 94, attended: "15 / 16", color: "green" }, { code: "CSE 4103", name: "Software Engineering", percent: 88, attended: "14 / 16", color: "green" }, { code: "CSE 4105", name: "Artificial Intelligence", percent: 81, attended: "13 / 16", color: "gold" }, { code: "HUM 4107", name: "Professional Ethics", percent: 100, attended: "10 / 10", color: "green" }];
  return <><PageHeader eyebrow="SPRING 2026 · WEEK 10" title="Attendance" description="Stay ahead of your attendance targets across every course." action={<button className="filter-button">Spring 2026 <ChevronDown size={15} /></button>} /><div className="attendance-banner"><div className="attendance-score"><div className="score-ring"><strong>91%</strong><span>overall</span></div><div><span className="eyebrow eyebrow-light">ATTENDANCE HEALTH</span><h2>You’re on track</h2><p>Keep above 80% to maintain your good standing.</p></div></div><div className="attendance-facts"><span><strong>52</strong> classes attended</span><span><strong>4</strong> absences this term</span></div></div><section className="surface-card attendance-card"><div className="table-heading"><div><span className="eyebrow">COURSE BREAKDOWN</span><h2>Attendance by course</h2></div><button className="icon-button"><MoreHorizontal size={18} /></button></div><div className="attendance-list">{courses.map((course) => <div className="attendance-row" key={course.code}><div className="attendance-course"><span className="course-square">{course.code.split(" ")[0]}</span><span><strong>{course.name}</strong><small>{course.code} · Spring 2026</small></span></div><div className="attendance-bar"><div><span>Attendance rate</span><strong>{course.percent}%</strong></div><div className="long-progress"><i className={course.color} style={{ width: `${course.percent}%` }} /></div></div><span className={`attendance-status ${course.color}`}>{course.percent >= 90 ? "Excellent" : "On watch"}</span><span className="attended-count">{course.attended}</span><button className="icon-button"><ArrowRight size={16} /></button></div>)}</div></section></>;
}

function Profile() {
  return <><PageHeader eyebrow="ACCOUNT & IDENTITY" title="Student profile" description="Your academic identity and contact information in one place." action={<button className="outline-button"><Settings size={16} /> Account settings</button>} /><div className="profile-layout"><section className="surface-card profile-identity"><div className="profile-cover"><div className="profile-avatar">FT</div></div><div className="profile-body"><h2>Farzana Tabassum</h2><p>Computer Science & Engineering · Level 4, Term 1</p><span className="verified-badge"><ShieldCheck size={14} /> Verified student</span><div className="identity-meta"><div><small>Student ID</small><strong>2021-CSE-042</strong></div><div><small>Batch</small><strong>2021 — 2026</strong></div></div></div></section><section className="surface-card details-card"><div className="table-heading"><div><span className="eyebrow">PERSONAL INFORMATION</span><h2>Contact details</h2></div><button className="text-button">Edit details</button></div><div className="details-grid"><div><small>University email</small><strong>farzana.tabassum@iut-dhaka.edu</strong></div><div><small>Phone number</small><strong>+880 1712 345 678</strong></div><div><small>Department</small><strong>Computer Science & Engineering</strong></div><div><small>Faculty</small><strong>Faculty of Science & Technical Education</strong></div><div><small>Date of birth</small><strong>14 February 2003</strong></div><div><small>Nationality</small><strong>Bangladeshi</strong></div></div></section><section className="surface-card advisor-card"><span className="eyebrow">ACADEMIC ADVISOR</span><div className="advisor-row"><div className="avatar avatar-medium">NR</div><div><strong>Dr. Nadia Rahman</strong><span>Associate Professor · CSE</span><small>nadia.rahman@iut-dhaka.edu</small></div><button className="icon-button"><ArrowRight size={16} /></button></div></section></div></>;
}
