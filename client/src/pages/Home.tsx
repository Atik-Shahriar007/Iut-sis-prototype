...
function Schedule() {
  ...
  return <><PageHeader eyebrow="SEMESTER 4 · 2025–26 · WEEK 10" title="Class schedule" description="Your week at a glance, with room and instructor details." action={<button className="outline-button"><CalendarDays size={16} /> Export calendar</button>} /><div className="schedule-tabs"><button className="active">Week view</button><button>Day view</button><span>08 — 12 March 2026 <ChevronDown size={15} /></span></div><section className="surface-card timetable-card"><div className="timetable"><div className="time-column"><span /><span>{blocks[0]}</span><span>{blocks[1]}</span><span>{blocks[2]}</span><span>{blocks[3]}</span><span>{blocks[4]}</span><span>{blocks[5]}</span></div>{days.map((day) => <div className="day-column" key={day}><div className="day-head"><strong>{day.slice(0, 3).toUpperCase()}</strong><span>{day === "Sunday" ? "08" : day === "Monday" ? "09" : day === "Tuesday" ? "10" : day === "Wednesday" ? "11" : "12"}</span></div><div className="day-grid"><div className={`calendar-class class-blue ${day === "Sunday" ? "placed" : "empty"}`}><strong>Algorithms</strong><span>09:00 — 10:30</span><small>AB3 · 402</small></div><div className={`calendar-class class-mint ${day === "Monday" ? "placed" : "empty"}`}><strong>Software Requirements & Specifications</strong><span>11:00 — 12:30</span><small>AB2 · 201</small></div><div className={`calendar-class class-gold ${day === "Tuesday" ? "placed" : "empty"}`}><strong>Probability & Statistics</strong><span>09:00 — 10:30</span><small>AB3 · 305</small></div><div className={`calendar-class class-rose ${day === "Wednesday" ? "placed" : "empty"}`}><strong>Database Management</strong><span>02:00 — 03:30</span><small>AB1 · 107</small></div></div></div>)}</div></section><div className="schedule-note"><Bell size={17} /><span><strong>Next class:</strong> Algorithms begins at 09:00 in Academic Block 3, Room 402.</span></div></>;
}
...
function Attendance() {
  const courses = [{ code: "SWE 2201", name: "Algorithms", percent: 94, attended: "15 / 16", color: "green" }, { code: "SWE 2203", name: "Software Requirements & Specifications", percent: 88, attended: "14 / 16", color: "green" }, { code: "MAT 2205", name: "Probability & Statistics", percent: 81, attended: "13 / 16", color: "gold" }, { code: "SWE 2207", name: "Database Management", percent: 100, attended: "10 / 10", color: "green" }];
